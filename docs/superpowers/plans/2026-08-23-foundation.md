...
- **Commit messages are written to spec.** Use the exact text given in each task's commit step. Lead with what changed, keep it short, no em dashes, no bold, no closing paragraph that restates the subject line. Never name the maintainer in a commit message, comment, or doc.
...
signal, so it goes deliberately rather than by accident.
