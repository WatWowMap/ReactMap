...
import AdvancedMenu from './AdvancedFilter.jsx'
import Tile from './PokemonTile.jsx'
import FilterOptions from '../components/FilterOptions'
import FilterFooter from '../components/FilterFooter'

const PokemonMenu = ({ settings, globalFilters, toggleDialog, availableForms }) => {
  ...
  const toggleAdvMenu = (open, id, filters) => (event) => {
    if (event.type === 'keydown' && (event.key === 'Tab' || event.key === 'Shift')) {
      return
    }
    if (open) {
      ...
      setTempFilters({ ...tempFilters, [id]: filters })
    }
  }
      ...
      {!advancedFilter.open ?
        <>
          ...
          <DialogContent>
            ...
            >
              {!isMobile && <Grid container item sm={4} md={3}
                spacing={2}
                direction="column"
                justify="flex-start"
                alignItems="flex-start"
        ...
        >
          {advancedFilter.open && <AdvancedMenu
