// server/acceptance/support/fake-golbat-server.ts
//
...
interface FakeGolbat {
  url: string
  setPokemonHandler: (fn: (body: any) => any) => void
  setFortHandler: (fn: (body: any) => any) => void
  setAvailablePokemonHandler: (fn: () => any) => void
  setAvailableGymsHandler: (fn: () => any) => void
  setAvailablePokestopsHandler: (fn: () => any) => void
  setAvailableStationsHandler: (fn: () => any) => void
  setAvailableFortsHandler: (fn: () => any) => void
  setStatus: (status: any) => void
  setSecret: (secret: string | null) => void
  getRequestLog: () => Array<{ path: string; body: any; at: number }>
  resetRequestLog: () => void
  close: () => void
}

function startFakeGolbat(): FakeGolbat {
  const requestLog: Array<{ path: string; body: any; at: number }> = []
  ...
  let pokemonHandler: (body?: any) => any = () => ({
  ...
  let availableStationsHandler: () => any = () => ({ battles: [] })

  // decoder/api_fort_available.go:10-14 (ApiAvailableForts). Gated.
  let availableFortsHandler: () => any = () => ({
