// server/acceptance/auth-flow.acceptance.ts
//
...
const usernameFor = (label: string) => `acceptance-${RUN_ID}-${label}`
...
const legacyUserIds: (string | number)[] = []
 ...
 */
async function waitForServerReady(maxWaitMs: number) {
...
beforeAll(async () => {
  // The suite's own header comment documents these as required: a real
  // MySQL reachable via these env vars, from .env.
...
describe('criterion 5: a revoked permission stops appearing on the next request', () => {
  ...
  let userId: string | undefined
  let permRowId: string | number | null
...
describe('criterion 6: a migrated legacy user can sign in with their existing password', () => {
  ...
  beforeAll(async () => {
    ...
    legacyUserIds.push((result as any).insertId)

    const backfillProcess = Bun.spawn({
      cmd: ['bun', 'server/src/scripts/backfill-auth-users.ts'],
      cwd: REPO_ROOT,
      env: process.env,
      stdout: 'pipe',
      stderr: 'pipe',
    ...
    if (exitCode !== 0) {
      ...
      throw new Error(
        `backfill-auth-users.ts exited ${exitCode}, expected 0:\n${stderr}`,
      )
    }
  }, 30_000)
