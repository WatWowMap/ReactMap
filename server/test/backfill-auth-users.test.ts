// server/test/backfill-auth-users.test.ts
//
...
import { isIdempotentConflict } from '../src/scripts/backfill-auth-users'

function dupEntryError(sqlMessage: string) {
  const error: Error & { code?: string; sqlMessage?: string } = new Error(
    sqlMessage,
  )
