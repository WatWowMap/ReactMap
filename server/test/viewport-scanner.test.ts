...
import { createGolbatClient } from '../src/services/golbat-client'
import {
  quarterBbox,
  scanFortsComplete,
  scanPokemonComplete,
} from '../src/services/viewport-scanner'
...
describe('subdivision against a fake Golbat', () => {
  let fakeGolbat: ReturnType<typeof startFakeGolbat>
  let client: ReturnType<typeof createGolbatClient>
  ...
  test('4: a limit_reached response subdivides, and the merged result is complete', async () => {
    let callCount = 0
    fakeGolbat.setPokemonHandler((body: any) => {
      callCount += 1
    ...
    expect(entities.map((e: any) => e.id).sort()).toEqual([
      'q-0-0',
      'q-0-1',
      'q-1-0',
      'q-1-1',
  ...
  test('forts: a truncated combined response subdivides and keeps gyms/pokestops/stations separate', async () => {
    fakeGolbat.setFortHandler((body: any) => {
      const isTopLevel =
        body.min.lat === 0 &&
        body.min.lon === 0 &&
        body.max.lat === 2 &&
