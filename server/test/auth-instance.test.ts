// server/test/authInstance.test.ts
...
test('a configured redirectUri is passed through to the discord provider', () => {
  ...
  expect(
    (buildAuthOptions(withRedirect).socialProviders.discord as any).redirectURI,
...
test('session.create.before and .after both wire up when both are provided', async () => {
  ...
  expect(
    await options.databaseHooks!.session.create.before!({ userId: 'u1' }),
  ).toBeUndefined()
  await options.databaseHooks!.session.create.after!({ userId: 'u1' })
  expect(recomputedFor as string | null).toBe('u1')
})
...
test('enforceMaxSessions is wired to session.create.after alongside onSessionCreate', async () => {
  let enforcedFor: string | null = null
  const options = buildAuthOptions({
    ...baseConfig,
    checkSignInGate: async () => ({ allow: true }),
