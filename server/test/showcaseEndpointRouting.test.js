...
test('Buddy Showcase filters stay on the exact Golbat endpoint path', async () => {
  const requests = []
  const ts = Math.floor(Date.now() / 1000)
  spyOn(scannerQuery, 'evalScannerQuery').mockImplementation(
    async (...request) => {
  ...
  spyOn(areaSql, 'getAreaSql').mockImplementation(() => true)
...
test('all-stop and AR-only modes are authorized at the model boundary', async () => {
  let endpointRows = []
  spyOn(scannerQuery, 'evalScannerQuery').mockImplementation(async () => ({
    pokestops: endpointRows,
  }))
  spyOn(areaSql, 'getAreaSql').mockImplementation(() => true)
  spyOn(questLayerMode, 'isDualQuestLayerMode').mockImplementation(() => true)
  ...
  try {
    ...
    assert.deepEqual(
      await Pokestop.getAll(eventOnlyPerms, args, baseContext),
      [],
    )
    assert.equal(
      (
    ...
    assert.deepEqual(
      await Pokestop.getAll(eventOnlyPerms, args, sqlContext),
      [],
    )
    assert.equal(
      (
