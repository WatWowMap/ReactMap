// server/test/authRecomputePerms.test.ts
import { expect, test } from 'bun:test'
import { computeUserPermsRows, derivedId } from '../src/auth/recompute-perms'
import {
  buildComputers,
  recomputeUserPerms,
} from '../src/auth/recompute-perms-on-sign-in'
...
test('recomputeUserPerms loads accounts, computes, and upserts', async () => {
  ...
  expect(upserted as any[] | null).toEqual([
    {
      id: derivedId('u1', 'discord'),
      userId: 'u1',
      providerId: 'discord',
...
test("buildComputers' discord computer honours an injected client + fetch implementation, passing the client and relevant guild ids straight through", async () => {
  ...
  const computers = buildComputers({
    ...
    getDiscordClient: () => ({ fake: true }) as any,
