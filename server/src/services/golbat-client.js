// @ts-check
// server/src/services/golbat-client.js
//
...
const { buildPokemonScanBody, buildFortScanBody } = require('./golbat-requests')
const {
  parseStatus,
...
} = require('./golbat-responses')

const DEFAULT_TIMEOUT_MS = 10_000

/**
 * @param {{fortInMemory: boolean} | null} capabilities
 * @param {string} path
 */
function assertFortInMemoryLocally(capabilities, path) {
 ...
 *   timeoutMs?: number,
 * }} [options] Every field overrides its `config`-backed default; intended
 *   for tests to inject a fake `fetchImpl` and skip `@rm/config` entirely.
 */
function createGolbatClient(options = {}) {
  ...
  /** @type {{fortInMemory: boolean, maxPokemonResults: number, maxFortResults: number} | null} */
  ...
  async function request(path, { method = 'GET', body } = {}) {
    ...
    try {
      ...
      throw new GolbatUnreachableError(
        `Golbat ${method} ${path} unreachable: ${/** @type {any} */ (e)?.message || e}`,
      )
    } finally {
      clearTimeout(timer)
    }
  ...
  return {
    ...
    /**
     * @param {{min: {lat:number,lon:number}, max: {lat:number,lon:number}, limit?: number, filters?: object[]}} params
     */
     ...
     *   limit?: number, withIncidents?: boolean,
...
module.exports = { createGolbatClient }
