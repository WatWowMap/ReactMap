// @ts-check
// server/src/services/rule-local-filter.js
//
...
/**
 * @param {number | null | undefined} ruleSpeciesId
 * @param {number | null | undefined} ruleFormId
 * @param {number | null | undefined} entitySpeciesId
 * @param {number | null | undefined} entityFormId
 * @returns {boolean}
 */
...
/**
 * @param {any[] | undefined} exclusions
 * @param {number | null | undefined} entitySpeciesId
 * @param {number | null | undefined} entityFormId
 * @returns {boolean}
 */
function isExcluded(exclusions, entitySpeciesId, entityFormId) {
 ...
 * @param {number | null | undefined} min
 * @param {number | null | undefined} max
 * @param {number | null | undefined} targetSpeciesId
 * @returns {boolean}
 */
function pvpRankMatches(entries, min, max, targetSpeciesId) {
 ...
 * @returns {boolean}
 */
function pokemonRuleMatches(rule, entity) {
...
/**
 * @param {any[]} rules Same shape `translatePokemonRules` takes: `rule`
 *   rows joined with `rule_pokemon`, plus `exclusions` on any rule whose own
 *   species is NULL.
 * @returns {(entity: any) => boolean}
 */
function buildPokemonLocalFilter(rules) {
  const list = rules ?? []
 ...
 * @returns {boolean}
 */
function gymRuleMatches(rule, entity) {
...
/**
 * @param {any[]} rules `rule` rows joined with `rule_gym`.
 * @returns {(entity: any) => boolean}
 */
function buildGymLocalFilter(rules) {
  const list = rules ?? []
  if (list.length === 0) return () => true
 ...
 * @returns {boolean}
 */
function pokestopRuleMatches(rule, entity) {
  switch (rule.role) {
    case 'quest': {
      if (
      ...
      for (const condition of rule.conditions ?? []) {
        ...
        const satisfied = conditions.some(
          (c) => c.title === condition.title && c.target === condition.target,
        )
    ...
    case 'invasion': {
      ...
      return invasions.some((incident) => {
        if (
    ...
    case 'event_stop':
      return (
        ...
        (entity.invasions ?? []).some(
          (incident) => incident.display_type === rule.event_display_type,
        )
      )
    default:
      return false
...
/**
 * @param {any[]} rules `rule` rows joined with `rule_pokestop` (and, for
 *   quest rules, `conditions`).
 * @returns {(entity: any) => boolean}
 */
function buildPokestopLocalFilter(rules) {
  const list = rules ?? []
  if (list.length === 0) return () => true
 ...
 * @returns {boolean}
 */
function stationRuleMatches(rule, entity) {
...
/**
 * @param {any[]} rules `rule` rows joined with `rule_station`.
 * @returns {(entity: any) => boolean}
 */
function buildStationLocalFilter(rules) {
  const list = rules ?? []
  if (list.length === 0) return () => true
 ...
 *
 * @param {{pokemon?: any[], gym?: any[], pokestop?: any[], station?: any[]}} rulesByCategory
 * @returns {{
