// @ts-check
// server/src/services/golbat-requests.js
//
 ...
 * @returns {number}
 */
function clampLimit(requested, cap) {
 ...
 *   filters?: object[],
 * }} params
 * @param {{maxPokemonResults?: number} | null} [caps] Capabilities read from
 *   `GET /api/status`; omit or pass `null` if not yet known (the request is
 *   still sent, and Golbat clamps server-side).
 */
function buildPokemonScanBody({ min, max, limit, filters }, caps) {
  return {
    min,
    max,
 ...
 * }} params Each of `gyms`/`pokestops`/`stations` opts that fort type into
 *   the result; omitting one (leaving it `undefined`/`null`) excludes it,
 *   per decoder/api_fort.go:55-59.
 * @param {{maxFortResults?: number} | null} [caps]
 */
function buildFortScanBody(
  { min, max, limit, withIncidents, gyms, pokestops, stations },
  caps,
) {
  return {
    min,
...
module.exports = { clampLimit, buildPokemonScanBody, buildFortScanBody }
