// @ts-check
// server/src/services/viewport-scanner.js
//
 ...
 * @returns {Array<{min: {lat:number,lon:number}, max: {lat:number,lon:number}}>}
 */
function quarterBbox(bbox) {
...
/**
 * @template T
 * @param {(bbox: {min: object, max: object}) => Promise<{leaf: T, limitReached: boolean}>} runQuery
 *   A single-bbox query, already bound to whatever filters/limit the
 *   caller wants. Returns one page's raw result plus `limitReached`,
 *   abstracted away from which Golbat endpoint it came from --
 *   `scanPokemonComplete`/`scanFortsComplete` below supply this and a
 *   `mergeLeaves` to combine pages of that shape.
 * @param {(leaves: T[]) => T} mergeLeaves Combines every leaf page (in
 *   subdivision order) into one merged result of the same shape.
 * @param {{min: {lat:number,lon:number}, max: {lat:number,lon:number}}} bbox
 * @param {{maxDepth?: number}} [options]
 * @returns {Promise<{result: T, complete: boolean, subdivided: boolean}>}
 */
async function scanComplete(runQuery, mergeLeaves, bbox, options = {}) {
  const maxDepth = options.maxDepth ?? DEFAULT_MAX_DEPTH

  async function recurse(box, depth) {
 ...
 * @param {{min: {lat:number,lon:number}, max: {lat:number,lon:number}}} bbox
 * @param {{filters?: object[], limit?: number}} [params]
 * @param {{maxDepth?: number}} [options]
 * @returns {Promise<{entities: any[], complete: boolean, subdivided: boolean}>}
 */
 ...
 * @param {import('./golbat-client').createGolbatClient extends (...args: any) => infer R ? R : never} golbatClient
 * @param {{min: {lat:number,lon:number}, max: {lat:number,lon:number}}} bbox
 * @param {{gyms?: object|null, pokestops?: object|null, stations?: object|null, limit?: number}} [params]
 * @param {{maxDepth?: number}} [options]
 * @returns {Promise<{gyms: any[], pokestops: any[], stations: any[], complete: boolean, subdivided: boolean}>}
 */
