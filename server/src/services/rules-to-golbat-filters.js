// @ts-check
// server/src/services/rules-to-golbat-filters.js
//
...
/** @param {any} rule */
function buildPokemonClause(rule) {
  const clause = {
...
/** @param {any} rule */
function pokemonLocalPredicates(rule) {
  const local = []
...
/**
 * @param {any[]} rules `rule` rows joined with `rule_pokemon` (and, for any
 *   rule whose own species is NULL, its `rule_exclusion` rows as `exclusions`).
 * @returns {{upstream: {filters: object[]} | null, local: object[]}}
 */
function translatePokemonRules(rules) {
...
/** @param {any} rule */
function gymLocalPredicates(rule) {
  const local = []
...
/** @param {any} rule */
function buildPokestopClause(rule) {
  const clause = {}
  switch (rule.role) {
    case 'quest': {
...
/** @param {any} rule */
function pokestopLocalPredicates(rule) {
  const local = []
...
/** @param {any} rule */
function buildStationClause(rule) {
  const clause = {}
...
/**
 * @param {any[]} rules
 * @param {(rule: any) => object} clauseBuilder
 * @param {(rule: any) => object[]} localBuilder
 * @returns {{group: {filters: object[]} | null, local: object[]}}
 */
function buildFortGroup(rules, clauseBuilder, localBuilder) {
...
/**
 * @param {{gym?: any[], pokestop?: any[], station?: any[]}} rulesByCategory
 * @returns {{
 *   upstream: {gyms: {filters: object[]} | null, pokestops: {filters: object[]} | null, stations: {filters: object[]} | null} | null,
 *   local: object[],
 * }}
 */
function translateFortRules({ gym = [], pokestop = [], station = [] } = {}) {
 ...
 *
 * @param {any[]} rules `rule` rows joined with `rule_nest`.
 * @returns {{upstream: null, local: object[]}}
 */
function translateNestRules(rules) {
