...
function verifyTelegramLogin(
  ...
  // A signed payload with no `id` is not a real login: the caller below
  // matches an account by this field, and an `undefined` accountId is not a
  // safe thing to hand to `findAccountByKey`. Typechecking this file as real
  // TypeScript (rather than never-checked JSDoc) is what surfaced that
  // `payload.id` was never actually guaranteed to exist.
  if (!payload.id) {
    return { ok: false as const, reason: 'bad-signature' }
  }
 ...
 *
 */
function telegramPlugin(options: { botToken: string }) {
...
export { DEFAULT_MAX_AGE_SECONDS, telegramPlugin, verifyTelegramLogin }
