// server/src/auth/discord-bot-client.js
// @ts-check
 ...
 */
function getSharedDiscordClient() {
 ...
 * @returns {import('discord.js').Client | null}
 */
function startDiscordBot({ botToken, onRoleChange }) {
  if (!botToken) {
    log.info(
      TAGS.auth,
 ...
 *
 * @returns {(discordUserId: string) => Promise<void>}
 */
function createOnRoleChange() {
  return async function onRoleChange(discordUserId) {
    const { createRevocationDeps } = require('./revoke-access-adapter')
...
module.exports = { startDiscordBot, getSharedDiscordClient, createOnRoleChange }
