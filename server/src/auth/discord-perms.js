 ...
 *
 * @param {{ id: string, guildResults: Record<string, DiscordGuildResult> | null }} user
 *   `guildResults` maps a guild id (every id in `rules.blockedGuilds` and
 *   `rules.allowedGuilds`, deduplicated) to what the bot found there. `null`
 *   means the bot itself was never queried at all (not configured) --
 *   equivalent to every entry being `unknown`.
 * @param {{
 *   allowedUsers: string[],
...
function computeDiscordPerms(user, rules) {
  ...
  const resultFor = (guildId) => user.guildResults?.[guildId] || null
